...
# data_apex_frame_path = 'datasets/data_all.csv'
data_apex_frame_path = 'datasets/data1_apex.csv'
# data_apex_frame_path = 'datasets/cohn.csv'
...
def load_me_data(data_root, file_path, subject_out_idx, batch_size=batch_size, num_workers=4):
    ...
    train_paths, train_labels = get_triple_meta_data_f(df_train_sampled)
    ...
    val_paths, val_labels = get_triple_meta_data_f(df_val)
...
def on_epoch(model, optimizer, lr_decay, train_loader, test_loader, epoch):
    ...
    with tqdm(total=steps) as progress_bar:  # jin du tiao
        ...
        for i, (x, y) in enumerate(train_loader):  # batch training
            ...
            y_pred = model(x, y)  # forward   [ 32, 3]   [32,2]
...
#     for i in range(16):  # 68 data_subs in total from 3 datasets + 98 data_subs from CK+
